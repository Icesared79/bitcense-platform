export function WhyUs() {
  ...
  return (
    <section id="features" className="py-32 px-4 sm:px-6 lg:px-8 bg-[#f8fafc] relative overflow-hidden">
      {/* Background decoration */}
      <div className="absolute top-1/4 right-0 w-96 h-96 bg-[#4A7C59]/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 left-0 w-96 h-96 bg-[#0f172a]/5 rounded-full blur-3xl" />
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {features.map((feature) => (
            <div
              key={feature.title}
              className="group bg-white rounded-3xl p-8 border border-gray-200 shadow-lg hover:shadow-2xl hover:shadow-gray-200/50 hover:border-gray-300 transition-all duration-300 hover:-translate-y-1"
            >
              <div className={`w-16 h-16 ${feature.iconBg} rounded-2xl flex items-center justify-center text-white mb-6 shadow-lg`}>
                {feature.icon}
              </div>
